; http://www.autohotkey.net/~Rseding91/Self%20Script%20Updater/Self%20Script%20Updater.ahk

Check_ForUpdate(Script_Name = "Self Script Updater", Version_Number = 1.7, Update_URL = "http://www.fake.net/Version.ini", Retry_Count = 3, _ReplaceCurrentScript = 1, _SuppressMsgBox = 0, _CallbackFunction = "", ByRef _Information = "")
{
	...
	Loop,% Retry_Count
	{
		...
		if(InStr(Update_URL, "http"))
			isURL := true

		if(isURL)
			UrlDownloadToFile,%Update_URL%,%Version_File%
		else
			FileCopy,%Update_URL%,%Version_File%
		...
		If (Version > Version_Number)
		{
			...
			{
				IniRead,Description,%Version_File%,Info,Description
				
				If (Description)
				{
					StringReplace, Description, Description, *, % "`n*", All
					MsgBox,0x4,New version available,There is a new version of %Script_Name% available.`nCurrent version: %Version_Number%`nNew version: %Version%`n`nPatch description:`n%Description%`n`nWould you like to download it now? Please note, the update process can take up to a few minutes.
				}
				Else
					MsgBox,0x4,New version available,There is a new version of %Script_Name% available.`nCurrent version: %Version_Number%`nNew version: %Version%`n`nWould you like to download it now? Please note, the update process can take up to a few minutes.
